...
const groundMaterial = new THREE.MeshStandardMaterial({ color: 0x808080 }); // Gray color
...
scene.add(groundMesh);
  ...
  const talkClip = THREE.AnimationClip.findByName(clips, "Explain");
  const stumbleClip = THREE.AnimationClip.findByName(clips, "Fight");
  ...
  container.addEventListener("mousedown", (ev) => {
    ...
    if (intersections.length > 0) {
      ...
      talkAction.crossFadeTo(stumbleAction, 0.3);
      ...
      setTimeout(() => {
        ...
        stumbleAction.crossFadeTo(talkAction, 1);
